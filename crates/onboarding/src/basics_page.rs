...
fn render_base_keymap_section(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    return v_flex().gap_2().child(Label::new("基础快捷键方案")).child(
...
fn render_vim_mode_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    SwitchField::new(
        "onboarding-vim-mode",
        Some("Vim 模式"),
        Some("从 Neovim 迁移而来？试试 Zed 原生支持的 Vim 模式。".into()),
        toggle_state,
        {
            let fs = <dyn Fs>::global(cx);
            move |&selection, _, cx| {
...
fn render_worktree_auto_trust_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    let tooltip_description = "将新项目标记为受信任后，Zed 才会运行语言服务器、项目设置和 MCP 服务器等服务。";

    SwitchField::new(
        "onboarding-auto-trust-worktrees",
        Some("默认信任所有项目"),
        Some("自动将所有新项目标记为受信任，以启用 Zed 的全部功能。".into()),
        toggle_state,
        {
            let fs = <dyn Fs>::global(cx);
            move |&selection, _, cx| {
